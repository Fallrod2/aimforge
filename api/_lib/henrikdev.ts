 ...
 *
 * La clé `HENRIKDEV_API_KEY` ne vit que dans l'environnement de la fonction
 * serverless. Elle n'est **pas** encore posée dans Vercel, et c'est un état
 * prévu, pas une panne : `isConfigured()` le dit, les fonctions répondent 503
 * avec un message net, et l'UI se dégrade au lieu de casser. Toute autre issue
 * (une clé vide envoyée quand même, un message d'erreur brut de l'API) ferait
 * passer un défaut de configuration pour un bug.
 *
...
/** La clé est-elle posée dans l'environnement de la fonction ? */
export function isConfigured(): boolean {
  return (process.env.HENRIKDEV_API_KEY ?? "").trim() !== "";
}

function apiKey(): string {
  const key = (process.env.HENRIKDEV_API_KEY ?? "").trim();

  if (key === "") throw new HenrikError(NOT_CONFIGURED, 503);
  return key;
}
...
export async function resolveAccount(name: string, tag: string): Promise<HenrikAccount> {
...
export async function fetchMmr(region: string, puuid: string): Promise<HenrikMmr> {
  const data = await getData(
